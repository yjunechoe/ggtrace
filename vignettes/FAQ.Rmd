...
But in case you're wondering, `{patchwork}` has `patchwork::wrap_ggplot_grob()` which allows a `<gtable>` to be properly aligned to other ggplots, so it's not totally limiting like a raster image of a figure.

```{r}
library(patchwork)
original_plot_titled <- original_plot + ggtitle("original plot")

# panels get aligned because `modified_plot` contains info about that
