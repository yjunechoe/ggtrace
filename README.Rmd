...
Here's another example where we make the method fit predictions from a loess regression instead. We could again use `ggtrace_highjack_return()` and in-line the calculation of the new return value using our captured function `captured_fn_2_3()`:
...
But you can also use `ggtrace_highjack_args()` to achieve the same without capturing the method as function first:
