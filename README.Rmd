...
The following code injects the code `z <- z * 10` right as `dummy_fn` enters the third "step" in the body, _right before_ the line `x <- 10` is ran.
...
The bulk of the work by a Stat is done in the `compute_*` family of methods. We'll focus on `compute_group` here, which you can look at with `get_method()`:
...
The answer is actually simple: it's an argument passed to `StatSmooth$compute_group`. We already saw its value briefly from `formals(captured_fn_2_3)`, but to target it explicitly we can also use `ggtrace_inspect_args()`:
...
Here's another example where we make the method fit predictions from a loess regression instead. To achieve this directly, we use `ggtrace_highjack_args()` and set the `values` to `list(method = "loess")`:
