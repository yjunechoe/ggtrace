...
- **User-friendly** ❤
    ...
    - The output is available for inspection outside of the debugging environment with `last_ggtrace()`
...
- **Flexible** 🛠
    ...
    - Since `ggtrace()` doesn't rely on interactivity, it's ideal for making `{reprex}`-es
    - Works with other object oriented systems in R (e.g., R6), not just ggproto!
    
- **Powerful** 💪
    - You can return the method's run time environment with `ggtrace()` for further inspection
