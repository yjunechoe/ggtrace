...
Then we target the same group inside `cond` and pass the modified value `modified_return_smooth` to the `value` argument using `substitute()`:
...
Note that violins are drawn with respect to the panel coordinate system, because that's how the `draw_group` method does things. We create and use the function `center_and_resize()` to edit the violins before they're passed to the `value` argument of `ggtrace_highjack_return()`.
